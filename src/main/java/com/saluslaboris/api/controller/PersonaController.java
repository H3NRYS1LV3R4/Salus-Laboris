...
@Validated
@PreAuthorize("hasRole('ADMIN') or hasAuthority('PAGE:/personas')")
public class PersonaController {
    ...
    @GetMapping("/{id}")
    public PersonaResponse obtener(@PathVariable @Positive Integer id) {
        return personas.obtener(id);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public PersonaResponse crear(@Valid @RequestBody PersonaRequest dto) {
        return personas.crear(dto);
    }
